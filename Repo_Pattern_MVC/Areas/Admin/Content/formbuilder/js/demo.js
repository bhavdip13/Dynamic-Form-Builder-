...
jQuery(function ($) {
	...
	var fbOptions = {
		...
		onSave: function (e, formData) {
			...
			$("#btnSaveForm").css("display", "block");
			$(".rendered-form").addClass("form-active");
			$("#txtFormName").css("display", "block");
   ...
   */
	...
	document.getElementById('edit-form').onclick = function () {
		toggleEdit();
		$("#btnSaveForm").hide();
		$(".rendered-form").removeClass("form-active");
		$("#txtFormName").css("display", "none");
	};
});

function SaveDynamicForm() {
	var formData = window.sessionStorage.getItem('formData');
	if ($("#txtFormName").val() != "" && $("#txtFormName").val() != undefined) {
		CheckFormnameExistfun($("#txtFormName").val());
		
		if ($("#hdnexist").val() == "False") {
			
			$.ajax({
				url: "/Formbuilder/SaveDyanamicForm",
				type: "POST",
				data: { FormName: $("#txtFormName").val(), Form_Json: JSON.stringify(formData), Form_HTML: $(".rendered-form").html() },
				dataType: "json",
				success: function (result) {

					if (result.Status == true) {

						setTimeout(function () { toastr.success(result.Message); }, 3000);
						window.sessionStorage.removeItem('formData');
			...
			toastr.warning("Sorry FormName, " + $("#txtFormName").val() + " is available in our database.");
		}
	}
	else {
...
function CheckFormnameExistfun(FormName) {
	
	
		$.ajax({
			url: "/Formbuilder/CheckFormnameExist",
			type: "POST",
			async: false,
